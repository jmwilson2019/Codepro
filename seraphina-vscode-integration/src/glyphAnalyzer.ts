...
/**
 * Glyph analysis = lightweight static pass + Grok narrative.
 * "Glyphs" are simply recognisable code patterns: control flow, async usage,
 * error handling, IO calls, etc. The static pass is deterministic and cheap.
 */
export class GlyphAnalyzer {
    ...
    async analyze(code: string, filePath: string, languageId?: string): Promise<GlyphReport> {
        ...
        const pushGlyph = (glyph: string, pattern: string, meaning: string) => {
            ...
            if (count > 0) {
                glyphs.push({ glyph, pattern, count, meaning });
            }
        };
    ...
    private async narrate(
        ...
        const client = this.pool.getClient(0);
        ...
        const prompt = `You are Seraphina's Glyph Interpreter. Given the static metrics and glyph pattern counts below for a ${language} file, write 3 short paragraphs (no bullet lists, no headings) describing:
