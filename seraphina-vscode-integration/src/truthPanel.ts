...
export class TruthPanel {
    ...
    static showOrUpdate(report: ConsensusReport, extensionUri: vscode.Uri): void {
        ...
        const panel = vscode.window.createWebviewPanel(
            'seraphinaTruthPanel',
            'Seraphina · Truth Validation',
            vscode.ViewColumn.Beside,
            { enableScripts: false, retainContextWhenHidden: true }
        );
        TruthPanel.current = new TruthPanel(panel, extensionUri);
        TruthPanel.current.update(report);
    }
    ...
    private constructor(panel: vscode.WebviewPanel, _extensionUri: vscode.Uri) {
        ...
        this.panel.onDidDispose(() => {
            this.disposed = true;
            if (TruthPanel.current === this) {
                TruthPanel.current = undefined;
            }
        });
    }

    update(report: ConsensusReport): void {
        this.panel.title = `Seraphina · ${report.filePath.split(/[\\/]/).pop()}`;
    ...
    private render(report: ConsensusReport): string {
        ...
        const scoreColor =
            report.truthScore >= 75 ? '#10b981' : report.truthScore >= 50 ? '#f59e0b' : '#ef4444';

        const wheel = this.renderRomanWheel(report);

        const agentCards = report.verdicts
            .map((v) => {
  ...
  <div class="grid">${agentCards}</div>
    ...
    private renderRomanWheel(report: ConsensusReport): string {
        ...
        const grid = [0.25, 0.5, 0.75, 1].map(
            (t) => `<circle cx="${cx}" cy="${cy}" r="${(r * t).toFixed(1)}" fill="none" stroke="currentColor" stroke-opacity="0.15" />`
        ).join('');
        ...
        const labels = points.map(
            (p) => `<text x="${p.labelX.toFixed(1)}" y="${p.labelY.toFixed(1)}" text-anchor="middle" dominant-baseline="middle" font-size="11" fill="currentColor">${escapeHtml(p.name)}</text>`
        ).join('');
        const dots = points.map(
            (p) => `<circle cx="${p.x.toFixed(1)}" cy="${p.y.toFixed(1)}" r="4" fill="currentColor" />`
        ).join('');
